...
EMASE <- function(SE, ma.N, preweights, smartass = FALSE) {
  ...
  fScores <- data.frame(t(apply(MASE, 1, model_weighting, trans="erfc", na.rm=TRUE)))
  if (!smartass) {
...
Regret <- function(SE, Mstar) {
  if (is.na(Mstar)) Mstar <- Reduce(min, SE[!is.na(SE)])
...
Learntse <- function(train,
  ...
  Mstar <- Reduce(max,
                  lapply(.tPerf, function(x) {
                    ae(y = x[,'trues'], y_hat = x[,'preds'])
                    })
                  )

  prew <- model_weighting(vnapply(.tPerf,
